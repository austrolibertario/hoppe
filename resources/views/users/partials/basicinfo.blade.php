    ...
    <div class="padding-sm user-basic-info">
        ...
        <div class="media">
            <div class="media-left">
              <div class="image">
                  ...
                  <a href="{{ route('users.edit_avatar', $user->id) }}" class="popover-with-html" data-content="Modificar avatar">
            ...
            <div class="media-body">
              ...
              <div class="item number">
                  Registrado em <span class="timeago">{{ $user->created_at }}</span>
              </div>

              @if($user->last_actived_at)
              <div class="item number">
                  Ativo em <span class="timeago">{{ $user->last_actived_at }}</span>
              </div>
              @endif
        ...
        <div class="follow-info row">
            <div class="col-xs-4">
              ...
              <a class="text" href="{{ route('users.followers', $user->id) }}">Seguidores</a>
            </div>
            <div class="col-xs-4">
                <a class="counter" href="{{ route('users.replies', $user->id) }}">{{ $user->reply_count +  $user->topic_count}}</a>
                <a class="text" href="{{ route('users.replies', $user->id) }}">Discussões</a>
            </div>
            <div class="col-xs-4">
                <a class="counter" href="{{ route('users.articles', $user->id) }}">{{ $user->article_count }}</a>
                <a class="text" href="{{ route('users.topics', $user->id) }}">Artigos</a>
            </div>
        </div>
        ...
        <div class="topic-author-box text-center">
            <ul class="list-inline">
              ...
              @if ($user->linkedin)
              <li class="popover-with-html" data-content="Visitar LinkedIn">
        ...
        @if ($currentUser && Entrust::can('manage_users') && $currentUser->id != $user->id && $currentUser->roles->count() < 5)
          ...
          <a class="btn btn-info btn-block" href="{{ url('admin/users/' . $user->id) }}" >
            <i class="fa fa-eye"></i> Administrar Usuários
          </a>
        @endif
...
@if ($currentUser && $currentUser->id == $user->id && Auth::user()->draft_count > 0)
    <div class="text-center alert alert-warning">
        <a href="{{ route('users.drafts') }}" style="color:inherit;"><i class="fa fa-file-text-o"></i> Rascunho {{ Auth::user()->draft_count }} 篇</a>
    </div>
@endif
   ...
   <div class="padding-sm user-basic-nav">
       <ul class="list-group">
             <a href="{{ route('users.articles', $user->id) }}" class="{{ navViewActive('users.articles') }}">
                 <li class="list-group-item"><i class="text-md fa fa-headphones"></i>Artigos</li>
             </a>

             <a href="{{ route('users.topics', $user->id) }}" class="{{ navViewActive('users.topics') }}">
                 <li class="list-group-item"><i class="text-md fa fa-list-ul"></i> Tópicos</li>
             </a>

             <a href="{{ route('users.replies', $user->id) }}" class="{{ navViewActive('users.replies') }}">
                 <li class="list-group-item"><i class="text-md fa fa-comment"></i> Respostas</li>
             </a>

             <a href="{{ route('users.following', $user->id) }}" class="{{ navViewActive('users.following') }}">
                 <li class="list-group-item"><i class="text-md fa fa-eye"></i> Seguidores</li>
             </a>

             <a href="{{ route('users.votes', $user->id) }}" class="{{ navViewActive('users.votes') }}">
                 <li class="list-group-item"><i class="text-md fa fa-thumbs-up"></i> Tópicos Curtidos</li>
             </a>
